...
Знаком (?) далее отмечены места, в которых я крайне не уверена, но решила проверить свое представление. Буду благодарна, если на ревью есть возможность указать верно ли я мыслю в этих моментах. Это мой первый опыт написания такого рода обращений к ревьюеру, надеюсь это дозволительно или хотя бы простительно.
...
4.(?) Кроме того, в проекте есть некоторые базовые компоненты, которые сложно отнести к тому или иному слою. В частности, брокер событий(?) - EventEmitter, который позволяет создавать события и подписываться на них.
...
* order - объект, представляющий информацию о текущем заказе, соответствует интерфейсу IOrderModel.
...
* getTotalPrice(): number - рассчитывает и возвращает общую стоимость товаров в корзине, суммируя цены всех товаров, идентификаторы которых находятся в корзине.
...
* catalog:changed - обновление списка товаров на странице
